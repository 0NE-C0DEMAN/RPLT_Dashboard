# RPLT Dashboard — GCP Deployment Guide

End-to-end instructions for deploying the RPLT Streamlit app to Google Cloud Platform. **Cloud Run** is the recommended target. Alternatives (App Engine, GKE, Compute Engine) are sketched at the bottom for context.
...
export REPO=rplt-dashboard
export SERVICE=rplt-dashboard
...
gcloud artifacts repositories create "$REPO" \
  ...
  --location="$REGION" \
  --description="RPLT Dashboard container images" \
  --project="$PROJECT_ID"
...
The URL comes back in the last line of output (`https://rplt-dashboard-xxxx.a.run.app`). That's it.
...
```bash
gcloud artifacts repositories create rplt-dashboard \
  --repository-format=docker \
...
gcloud builds submit \
  --tag us-central1-docker.pkg.dev/$PROJECT_ID/rplt-dashboard/rplt-dashboard:latest
```
...
```bash
docker build -t us-central1-docker.pkg.dev/$PROJECT_ID/rplt-dashboard/rplt-dashboard:latest .
docker push    us-central1-docker.pkg.dev/$PROJECT_ID/rplt-dashboard/rplt-dashboard:latest
```
...
```bash
gcloud run deploy rplt-dashboard \
  --image=us-central1-docker.pkg.dev/$PROJECT_ID/rplt-dashboard/rplt-dashboard:latest \
  --region=us-central1 \
  --platform=managed \
  --allow-unauthenticated \
  --memory=2Gi --cpu=2 \
...
- Substitutions (override if needed):
  - `_REGION`: `us-central1`
  - `_REPO`: `rplt-dashboard`
  - `_SERVICE`: `rplt-dashboard`
...
```bash
gcloud run revisions list --service=rplt-dashboard --region=us-central1
gcloud run services update-traffic rplt-dashboard \
  --region=us-central1 \
  --to-revisions=rplt-dashboard-00034-abc=100
```
...
gcloud run services remove-iam-policy-binding rplt-dashboard \
  --region=us-central1 --member=allUsers --role=roles/run.invoker

# Grant access to specific users / groups
gcloud run services add-iam-policy-binding rplt-dashboard \
  --region=us-central1 \
...
1. Create a bucket: `gsutil mb -l us-central1 gs://rplt-dashboard-data`
2. Mount it with Cloud Storage FUSE — add to `Dockerfile`:
   ...
   ```bash
   gcsfuse --implicit-dirs rplt-dashboard-data /app/data && streamlit run ...
   ```
...
gcloud run deploy rplt-dashboard --update-secrets=GSA_JSON=google-service-account:latest ...
```
...
Cloud Run ships logs to Cloud Logging automatically (view in the console under **Cloud Run → rplt-dashboard → Logs**). For structured logging, use `structlog` or `logging` with a JSON formatter. For metrics + alerts, Cloud Monitoring pulls revision / request / CPU metrics from Cloud Run out-of-the-box.
...
```bash
# Build
docker build -t rplt-dashboard .

# Run (Cloud Run's default port)
docker run --rm -p 8080:8080 -e PORT=8080 rplt-dashboard
...
```bash
gcloud compute instances create-with-container rplt-dashboard \
  --machine-type=e2-medium \
  --container-image=us-central1-docker.pkg.dev/$PROJECT_ID/rplt-dashboard/rplt-dashboard:latest \
