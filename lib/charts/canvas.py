...
from lib.tokens import (
    ACCENT, BG_SOFT, BORDER, NAVY, NAVY_LIGHT, SURFACE,
    TEXT, TEXT_2, TEXT_3,
)
...
def chart_panel(
    ...
    *,
    ...
    # Header (46) + toolbar row (38) + canvas (height) + padding (20).
    total_height = 46 + 38 + height + 20
...
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .small-btn.active {{
    /* ui.jsx SmallBtn active: navy bg, no border, weight 600.
       NAVY_LIGHT instead of NAVY because on dark theme, pure NAVY
       (#0f1729) is almost identical to SURFACE (#0a0f18) and doesn't
       stand out. NAVY_LIGHT (#1a2744) gives the "raised active" pop. */
    background: {NAVY_LIGHT}; color: #fff;
  ...
  .tt {{
    /* charts.jsx tooltip: navy bg, radius 9, padding 10 14, shadow 0.25 */
    position: absolute; background: {NAVY}; border-radius: 9px;
  ...
  .tt-lbl {{ font-size: 11px; color: #94a3b8; flex: 1; }}
  .tt-val {{
